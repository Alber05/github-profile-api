...
function InfoCard({ title, contain, name, setSection = null, section }) {
  ...
  return (
    <div
      className={`userInfo-card relative flex h-max w-full flex-wrap items-center overflow-hidden rounded-lg bg-customNavyBlue--dark p-4 text-xs ${section ? 'active' : null} ${!contain || title == 'Ubicación' ? null : 'cursor-pointer'}`}
      onClick={handleClick}
    >
      <span></span>
      <span></span>
